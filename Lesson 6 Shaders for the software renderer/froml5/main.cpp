...
struct GroudaudShader : public IShader{
    Vec3f varying_intensity;
    virtual Matrix vertex(int iface, int nthvert){
        Vec3f v = model->vert(iface, nthvert);
        Matrix m(v);
        m = ViewPort*Projection*View*m;
...
int main(int argc, char** argv) {
    ...
    Projection = creatProjection(-1.f/(center-eye).norm());
    ViewPort = createViewPort(width_2, height_2, 255);
    View = createModelView(eye, center, Vec3f(0,1,0)); 
    // creatProjection(-1.f/(center-eye).norm());
    // createViewPort(width_2, height_2, 255);
    // createModelView(eye, center, Vec3f(0,1,0));
    ...
    for (int i=0; i<model->nfaces(); i++) {
        ...
        for (int j=0; j<3; j++) {
            ...
            //should add float_err, implement in converting Vec3f to Vec3i.
